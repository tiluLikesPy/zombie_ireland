...
public class FPSCharacter : MonoBehaviour
{
    ...
    void Update()
    {
        Vector3 amoController = new Vector3(0F, 0F, -100F);
        if (Input.GetMouseButtonDown(0)){

                GameObject go = GameObject.Instantiate(ammunition);

                go.transform.position = ammunition.transform.position;
            go.transform.localScale = new Vector3(1, 1, 1);
            //ammunition.transform.Translate(amoController * Time.deltaTime);
